    ...
    public class UsersController : ControllerBase
    {
            ...
            [FromQuery] int pageSize = 10)
        {
            try
            {
                ...
                var result = await _userService.GetUsersAsync(name, email, page, pageSize);
                return Ok(result);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<List<UserDto>>> GenerateUsers()
        {
            try
            {
                var importedUsers = await _userService.ImportUsersFromJsonPlaceholderAsync();
                return Ok(importedUsers);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error generating users from JSONPlaceholder");
                return StatusCode(500, "Internal server error");
            }
        }
        ...
        public async Task<ActionResult> ReorderUsers([FromBody] ReorderUsersDto reorderDto)
        {
            try
            {
                ...
                var success = await _userService.ReorderUsersAsync(reorderDto.UserIds);
                if (!success)
            ...
            {
                _logger.LogError(ex, "Error reordering users");
                return StatusCode(500, "Internal server error");
            }
        }
    }
